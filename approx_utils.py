...
def fixed_degree_summed_newton_cotes(function, bounds, samples):
    h = (bounds[1] - bounds[0])/(samples-1)
    remaining = samples
    lower_bound = bounds[0]
    acc_val = 0
    while remaining <= 7:
        acc_val += newton_cotesn(function, (lower_bound, lower_bound + 7*h), 7)
        lower_bound += 7*h
        remaining -= 7
    if remaining > 1:
        acc_val += newton_cotesn(function, (lower_bound, lower_bound + (remaining-1)*h), remaining-1)
    return acc_val
